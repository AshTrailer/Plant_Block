...
def checkNtpAndSntp(host: str):
   """检测NTP和SNTP服务是否可用，并判断与目标机器的时差"""
   ntpClient = ntplib.NTPClient()

   logger.info(f"Checking Standard NTP service on {host}...")
   try:
      # 标准NTP请求通常对应版本3
      ntpResponse = ntpClient.request(host, version=3, timeout=5)
      logger.info(f"✅ NTP is available! Time offset (Target - Local): {ntpResponse.offset:.4f} seconds")
   except Exception as e:
      logger.warning(f"❌ NTP check failed: {e}")

   logger.info(f"Checking SNTP service on {host}...")
   try:
      # SNTP请求通常可以使用版本4
      sntpResponse = ntpClient.request(host, version=4, timeout=5)
      logger.info(f"✅ SNTP is available! Time offset (Target - Local): {sntpResponse.offset:.4f} seconds")
   except Exception as e:
      logger.warning(f"❌ SNTP check failed: {e}")
...
class SimulatedIoTDevice:
   def __init__(self, deviceId: str, brokerUrl: str, keepAliveParam: int):
      self.deviceId = deviceId
      self.brokerUrl = brokerUrl

      # 配置 AMQTT 客户端
      # [修改] 1. 显式传入 client_id，防止生成随机 ID
      # [修改] 2. will.message 必须是 bytes (加上前缀 b)，否则 amqtt 会静默忽略遗嘱配置
      self.client = MQTTClient(client_id=self.deviceId, config={
         'keep_alive': keepAliveParam,
         'auto_reconnect': True,
         'default_qos': 1,
         'will': {
            'topic': f'registry/{self.deviceId}/status',
            'message': 'offline', # 此处不可使用bytes，amqtt要求必须是str
            'qos': 1,
            'retain': False
         }
      })
...
if __name__ == "__main__":
   checkNtpAndSntp(SERVER_HOST)
   device = SimulatedIoTDevice(DEVICE_ID, BROKER_URL, keepAlive)
   try:
